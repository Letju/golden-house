...
\fancyhead[R]{\footnotesize\color{gray}Polytech Montpellier -- IWA 2 (S9)}
...
% =============================================================================
% EN-TÊTE TECHNIQUE (Page 1)
% =============================================================================
\noindent
\begin{minipage}[c]{0.65\textwidth}
    {\LARGE\bfseries\color{primary} GOLDEN HOUSE}\\[2pt]
    {\large\bfseries\color{secondary} Architecture Microservices \& Spécifications Techniques}\\[3pt]
    {\footnotesize\textbf{Auteurs :} Justin Chapon, Nabil Bennacer, Tom Cantillon, Salma El Annaby, Manuela Zapata Quirós}
\end{minipage}
\hfill
\begin{minipage}[c]{0.32\textwidth}
    \raggedleft\footnotesize
    \textbf{Polytech Montpellier} -- IG5 / DaMS\\
    Module : \textbf{IWA 2} (Christophe Nauroy)\\
...
% =============================================================================
% SECTION 1 : SCHÉMA D'ARCHITECTURE TIKZ (Page 1)
% =============================================================================
\section{Schéma d'Architecture Globale des Microservices}
...
% =============================================================================
\section{Typologie des Échanges Inter-Services}

\begin{itemize}[leftmargin=*, nosep]
    \item \textbf{\color{syncblue}Communication Synchrone (REST / HTTP)} : Requêtes bloquantes exigeant une réponse immédiate pour finaliser l'action courante (lecture temps réel du catalogue, calcul et figeage du panier, validation de l'adresse acheteur, réservation atomique des stocks, création de session Stripe).
    \item \textbf{\color{asyncorange}Communication Asynchrone (Events / Webhooks)} : Traitements découplés dans le temps et tolérants aux pannes sans bloquer l'acheteur (réception webhook Stripe, confirmation de paiement vers commande, génération différée de la facture légale PDF, notifications email/push, réassort fournisseur, recalcul de note).
\end{itemize}
...
% =============================================================================
\section{Matrice Exhaustive des Flux Inter-Services}

\begin{center}
\small
\begin{tabularx}{\textwidth}{@{} l l c p{3.2cm} X @{}}
\toprule
\textbf{Source} & \textbf{Cible} & \textbf{Type} & \textbf{Protocole / Signal} & \textbf{Justification Technique \& Métier} \\
\midrule
\textbf{Clients (Mobile/Web)} & API Gateway & \textcolor{syncblue}{\textbf{Sync}} & HTTP/2 / REST & Point d'entrée unique, terminaison TLS, validation JWT. \\
\textbf{API Gateway} & Microservices & \textcolor{syncblue}{\textbf{Sync}} & HTTP REST interne & Routage inverse vers les services cibles selon l'URI. \\
\textbf{Chatbot IA} & MCP Gateway & \textcolor{syncblue}{\textbf{Sync}} & JSON-RPC / REST & Traduction du langage naturel en requêtes microservices. \\
\addlinespace[2pt]
\textbf{Order} & User & \textcolor{syncblue}{\textbf{Sync}} & \texttt{GET /api/users/\{id\}} & Contrôle bloquant de l'existence client et adresse de livraison. \\
\textbf{Order} & Cart & \textcolor{syncblue}{\textbf{Sync}} & \texttt{GET /api/cart/\{id\}} & Récupération synchrone du snapshot panier (items, totaux) à figer. \\
\textbf{Cart} & Catalog & \textcolor{syncblue}{\textbf{Sync}} & \texttt{GET /api/products/\{id\}} & Vérification en temps réel du stock disponible et du prix unitaire. \\
\textbf{Order} & Catalog & \textcolor{syncblue}{\textbf{Sync}} & \texttt{POST /products/reserve} & Verrouillage atomique des stocks avant confirmation du devis. \\
\textbf{Order} & Payment & \textcolor{syncblue}{\textbf{Sync}} & \texttt{POST /payments/init} & Création de la session d'encaissement et obtention URL Stripe. \\
\textbf{Payment} & Stripe & \textcolor{syncblue}{\textbf{Sync}} & HTTPS / REST & Appel synchrone à l'API Stripe pour initialiser le Checkout. \\
\textbf{Review} & Catalog & \textcolor{syncblue}{\textbf{Sync}} & \texttt{GET /api/products/\{id\}} & Validation d'existence du produit avant enregistrement de l'avis. \\
\addlinespace[2pt]
\textbf{Stripe} & Payment & \textcolor{asyncorange}{\textbf{Async}} & Webhook \texttt{checkout.session} & Signal bancaire externe reçu hors session utilisateur. \\
\textbf{Payment} & Order & \textcolor{asyncorange}{\textbf{Async}} & Event \texttt{PaymentConfirmed} & Notification de règlement : transition de commande vers \texttt{PAID}. \\
\textbf{Payment} & Billing & \textcolor{asyncorange}{\textbf{Async}} & Event \texttt{OrderPaidEvent} & Déclenchement autonome de la génération de facture (non bloquant). \\
\textbf{Billing} & Notification & \textcolor{asyncorange}{\textbf{Async}} & Event \texttt{InvoiceGenerated} & Transmission de l'URL du PDF de facture pour envoi à l'acheteur. \\
\textbf{Order} & Notification & \textcolor{asyncorange}{\textbf{Async}} & Event \texttt{OrderStatusChanged} & Notification client des étapes de traitement (expédiée, livrée). \\
\textbf{Payment} & Notification & \textcolor{asyncorange}{\textbf{Async}} & Event \texttt{PaymentSucceeded} & Envoi immédiat de l'email de confirmation d'achat avec le reçu. \\
\textbf{Catalog} & Notification & \textcolor{asyncorange}{\textbf{Async}} & Event \texttt{StockBelowThreshold} & Alerte automatique du gestionnaire en cas de stock critique. \\
\textbf{Catalog} & Supplier & \textcolor{asyncorange}{\textbf{Async}} & Event \texttt{RestockNeededEvent} & Émission automatique d'une demande de réapprovisionnement. \\
\textbf{Supplier} & Catalog & \textcolor{asyncorange}{\textbf{Async}} & Event \texttt{StockReceivedEvent} & Incrémentation asynchrone des stocks à réception fournisseur. \\
\textbf{Review} & Supplier & \textcolor{asyncorange}{\textbf{Async}} & Event \texttt{RatingAggregated} & Recalcul asynchrone de la note de qualité moyenne fournisseur. \\
\textbf{Batches} & Order / Catalog & \textcolor{asyncorange}{\textbf{Async}} & Jobs Spark SQL & Traitements nocturnes pour agrégats KPIs et recommandations ALS. \\
\bottomrule
\end{tabularx}
\end{center}
...
% =============================================================================
\section{Points d'Entrée \& Modèles de Données -- Services 1 à 5}

\subsection{API Gateway \& MCP Gateway (Passerelles d'accès)}
\begin{itemize}[leftmargin=*, nosep]
    ...
    \end{itemize}
    \item \textbf{Modèle de Données} : \textit{Stateless}. Cache Redis pour la révocation des tokens JWT et le rate limiting.
\end{itemize}

\vspace{2pt}
\subsection{User Service (Utilisateurs, Adresses \& Authentification)}
\begin{itemize}[leftmargin=*, nosep]
    \item \textbf{Points d'Entrée} :
    \begin{itemize}[nosep]
    ...
    \end{itemize}
    \item \textbf{Modèle de Données (PostgreSQL)} :
    \begin{itemize}[nosep]
...
\vspace{2pt}
\subsection{Catalog Service (Produits, Mobilier \& Gestion des Stocks)}
\begin{itemize}[leftmargin=*, nosep]
    \item \textbf{Points d'Entrée} :
    \begin{itemize}[nosep]
    ...
    \item \textbf{Modèle de Données (PostgreSQL)} :
    \begin{itemize}[nosep]
        \item \texttt{\textbf{products}} : \texttt{id} (UUID, PK), \texttt{nom} (VARCHAR), \texttt{prix} (NUMERIC), \texttt{categorie} (category\_enum), \texttt{type\_matiere} (material\_enum), \texttt{couleur}, \texttt{largeur/hauteur/prof.} (INT), \texttt{stock} (INT), \texttt{seuil\_alerte} (INT), \texttt{images} (TEXT[]), \texttt{fournisseur\_id} (UUID, FK), \texttt{est\_actif} (BOOL).
    \end{itemize}
\end{itemize}

\vspace{2pt}
\subsection{Cart Service (Gestion des Paniers d'Achat)}
\begin{itemize}[leftmargin=*, nosep]
    \item \textbf{Points d'Entrée} :
    \begin{itemize}[nosep]
...
\vspace{2pt}
\subsection{Order Service (Commandes \& Cycle de Vie)}
\begin{itemize}[leftmargin=*, nosep]
    \item \textbf{Points d'Entrée} :
    \begin{itemize}[nosep]
...
% =============================================================================
\section{Points d'Entrée \& Modèles de Données -- Services 6 à 10}

\subsection{Payment Service (Passerelle Stripe \& Encaissements)}
\begin{itemize}[leftmargin=*, nosep]
    \item \textbf{Points d'Entrée} :
    \begin{itemize}[nosep]
...
\vspace{2pt}
\subsection{Billing Service (Facturation \& Génération Fiscale)}
\begin{itemize}[leftmargin=*, nosep]
    \item \textbf{Composant Métier} : Consommateur asynchrone de l'événement \texttt{OrderPaidEvent}. Attribue un numéro fiscal chronologique unique, calcule la ventilation HT / TVA / TTC, et compile le document légal PDF stocké sur S3 / MinIO.
    \item \textbf{Points d'Entrée} :
    \begin{itemize}[nosep]
...
\vspace{2pt}
\subsection{Notification Service (Emails SMTP \& Push Mobiles)}
\begin{itemize}[leftmargin=*, nosep]
    \item \textbf{Points d'Entrée} :
    \begin{itemize}[nosep]
...
\vspace{2pt}
\subsection{Review Service \& Batches Spark (Avis Clients \& Analyse Données)}
\begin{itemize}[leftmargin=*, nosep]
    \item \textbf{Review Service (Avis Clients)} :
    \begin{itemize}[nosep]
    ...
    \end{itemize}
    \item \textbf{Batches Spark (Moteur Analytique \& Recommandation)} :
    \begin{itemize}[nosep]
